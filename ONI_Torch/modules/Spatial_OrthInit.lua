local Spatial_OrthInit, parent =
    torch.class('cudnn.Spatial_OrthInit', 'nn.SpatialConvolution')
local ffi = require 'ffi'
local find = require 'cudnn.find'
local errcheck = cudnn.errcheck
local checkedCall = find.checkedCall

function Spatial_OrthInit:__init(nInputPlane, nOutputPlane,
...
function Spatial_OrthInit:resetWeightDescriptors(desc)
    -- for compatibility
...
function Spatial_OrthInit:fastest(mode)
...
function Spatial_OrthInit:setMode(fmode, bdmode, bwmode)
    if fmode ~= nil then
        self.fmode = fmode
    end
    if bdmode ~= nil then
...
function Spatial_OrthInit:resetMode()
    self.fmode = nil
    self.bdmode = nil
    self.bwmode = nil
    return self
...
function Spatial_OrthInit:noBias()
   self.bias = nil
   self.gradBias = nil
   return self
end
...
function Spatial_OrthInit:checkInputChanged(input)
...
function Spatial_OrthInit:createIODescriptors(input)
   local batch = true
   if input:dim() == 3 then
      input = input:view(1, input:size(1), input:size(2), input:size(3))
      batch = false
   end
   if Spatial_OrthInit.checkInputChanged(self, input) then
...
function Spatial_OrthInit:updateOutput(input)
...
function Spatial_OrthInit:updateGradInput(input, gradOutput)
...
function Spatial_OrthInit:accGradParameters(input, gradOutput, scale)
...
function Spatial_OrthInit:clearDesc()
    self.weightDesc = nil
    self.biasDesc = nil
    self.convDesc = nil
    self.iDesc = nil
...
function Spatial_OrthInit:write(f)
    self:clearDesc()
    local var = {}
...
function Spatial_OrthInit:clearState()
   self:clearDesc()
...
local function viewWeight(self)
  ...
  function Spatial_OrthInit:reset_orthogonal()
