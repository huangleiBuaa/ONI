local Spatial_OrthReg_OI, parent =
    torch.class('cudnn.Spatial_OrthReg_OI', 'nn.SpatialConvolution')
local ffi = require 'ffi'
local find = require 'cudnn.find'
local errcheck = cudnn.errcheck
local checkedCall = find.checkedCall

function Spatial_OrthReg_OI:__init(nInputPlane, nOutputPlane,
    ...
    self.eye=torch.eye(nOutputPlane):cuda()
 ...
 local function viewWeight(self)
  ...
  local function unviewWeight(self)
   ...
   function Spatial_OrthReg_OI:reset_orthogonal()
...
function Spatial_OrthReg_OI:resetWeightDescriptors(desc)
    -- for compatibility
...
function Spatial_OrthReg_OI:fastest(mode)
...
function Spatial_OrthReg_OI:setMode(fmode, bdmode, bwmode)
    if fmode ~= nil then
        self.fmode = fmode
    end
    if bdmode ~= nil then
...
function Spatial_OrthReg_OI:resetMode()
    self.fmode = nil
    self.bdmode = nil
    self.bwmode = nil
    return self
...
function Spatial_OrthReg_OI:noBias()
   self.bias = nil
   self.gradBias = nil
   return self
end
...
function Spatial_OrthReg_OI:checkInputChanged(input)
...
function Spatial_OrthReg_OI:createIODescriptors(input)
   local batch = true
   if input:dim() == 3 then
      input = input:view(1, input:size(1), input:size(2), input:size(3))
      batch = false
   end
   if Spatial_OrthReg_OI.checkInputChanged(self, input) then
...
function Spatial_OrthReg_OI:updateOutput(input)
...
function Spatial_OrthReg_OI:updateGradInput(input, gradOutput)
...
function Spatial_OrthReg_OI:accGradParameters(input, gradOutput, scale)
...
function Spatial_OrthReg_OI:clearDesc()
    self.weightDesc = nil
    self.biasDesc = nil
    self.convDesc = nil
    self.iDesc = nil
...
function Spatial_OrthReg_OI:write(f)
    self:clearDesc()
    local var = {}
...
function Spatial_OrthReg_OI:clearState()
   self:clearDesc()
...
function Spatial_OrthReg_OI:updateWeight(beta)
--    backCompatibility(self)
--    viewWeight(self)
--    print('----------update weight matrix-----------')
 
    local filters=self.weight:clone():view(self.weight:size(1),-1)
    local grad=torch.mm(filters,filters:transpose(2,1))
    grad:add(-self.eye)
--    print(grad)
    grad=torch.mm(grad,filters)
    local weight=self.weight:view(self.weight:size(1),-1)
    local coef=beta*-1
    weight:add(grad*coef)
