...
function Spatial_Weight_BN:__init(nInputPlane, nOutputPlane,flag_adjustScale, N_scale,
...
-----------------------------transform--------------------------
  ...
  self.gradWeight:cmul(self.W_hat, self.gradW)
     ...
     self.gradWeight:cmul(self.W_hat):mul(-1)
     self.gradWeight:add(self.gradW)
    
     self.gradWeight:cmul(self._scale:expandAs(self.gradWeight))
     if self.flag_adjustScale then
...
function Spatial_Weight_BN:parameters()
   if self.flag_adjustScale then
...
function Spatial_Weight_BN:endTraining()
    viewWeight(self)
     viewW(self)
      local n_output=self.weight:size(1)
     local n_input=self.weight:size(2)
