...
local function createModel(opt)
   ...
   local function basicblock(n, stride)
      ...
      s:add(cudnn.Spatial_Weight_DBN_PowerIter_F2(nInputPlane,n,opt.m_perGroup, opt.nIter,true, opt.N_scale,3,3,stride,stride,1,1))
   ...
   local model = nn.Sequential()
     assert((depth - 2) % 3 == 0, 'depth should be 3n+2')
      ...
      iChannels = 16*k
       ...
       model:add(cudnn.Spatial_Weight_DBN_PowerIter_F2(3,16*k,opt.m_perGroup,opt.nIter, true,opt.N_scale,3,3,1,1,1,1))
      model:add(ReLU(true))
