...
require 'cunn'
require 'cudnn'
require '../../modules/Spatial_GroupNormalization'
local Convolution = cudnn.SpatialConvolution
local Avg = cudnn.SpatialAveragePooling
local ReLU = cudnn.ReLU
local Max = nn.SpatialMaxPooling
local SGroupNorm = nn.Spatial_GroupNormalization
...
local function createModel(opt)
   ...
   local function shortcut(nInputPlane, nOutputPlane, stride)
      ...
      if useConv then
         ...
         return nn.Sequential()
            ...
            :add(SGroupNorm(nOutputPlane,opt.m_perGroup))
   ...
   local function basicblock(n, stride)
      ...
      s:add(SGroupNorm(n,opt.m_perGroup))
   ...
   local model = nn.Sequential()
      ...
      model:add(SGroupNorm(16*k,opt.m_perGroup))
      if opt.dropout > 0 then
           --model:add(Dropout())
   ...
   BNInit('nn.Spatial_GroupNormalization',1)
