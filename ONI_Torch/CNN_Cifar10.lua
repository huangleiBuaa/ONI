...
cmd:text()
cmd:text()
cmd:text('Validate the ONI on VGG Style Networks')
cmd:text()
cmd:text('Options')
...
cmd:option('-save',"log_exp_ONI_Cifar10" ,'subdirectory to save logs')
...
cmd:option('-learningRateDecayRatio',0.2,'Learning rate decay ratio')
cmd:option('-weightDecay',0.0005,'weight Decay for regularization')
cmd:option('-dampening',0,'')
cmd:option('-momentum',0.9,'momentum')
--cmd:option('-eps',1e-5,'the revisation')
...
cmd:option('-N_scale',1.414,'1; 1.414')
cmd:option('-nIter',  5,'Iteration Number for ONI')
cmd:option('-m_perGroup',64,'the channel number of per group')
...
opt.rundir = cmd:string('console/exp_0_vggStyle_Cifar10/Info', opt, {dir=true})
...
local net = dofile('models/CNN/'..opt.model..'.lua'):cuda()
  -- print('-------------------')
do
...
baseString=opt.model..'_depth'..opt.depth
         ...
         ..'_G'..opt.m_perGroup..'_b'..opt.batchSize..'_wf'..opt.widen_factor..'_wD'..opt.weightDecay..'_NScale'..opt.N_scale
         ..'_nIter'..opt.nIter..'_lD'..opt.learningRateDecayRatio..'_dr'..opt.dropout
         ..'_seed'..opt.seed
...
function train()
  ...
  for t,v in ipairs(indices) do
    ...
    if string.match(opt.model,'OrthReg') then     
        for k,v in pairs(model:findModules('cudnn.Spatial_OrthReg_OI')) do
         --print('--------OrthReg_UptateWeight-----') 
         v:updateWeight(opt.weightDecay)
        end
    end
...
for epoch=1,opt.max_epoch do
  ...
  log{
    ...
    torch.save('./set_result/result_Cifar10_Now_'..baseString..'.dat',results)
end
if opt.modelSave==1 then
-- Weight normalization method, get the weight matrix 
    for k,v in pairs(model:findModules('cudnn.Spatial_Weight_DBN_PowerIter_F2_CWN')) do
        v:endTraining()
    end
