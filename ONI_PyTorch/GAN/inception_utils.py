...
def accumulate_inception_activations_onlyZ(sample, net, num_inception_images=50000):
  pool, logits = [], []
  while (torch.cat(logits, 0).shape[0] if len(logits) else 0) < num_inception_images:
    with torch.no_grad():
      images = sample()
...
def prepare_inception_metrics(dataset, parallel, no_fid=False, onlyZ=True):
  ...
  data_mu_train = np.load(dataset+'_inception_moments_train.npz')['mu']
  data_sigma_train = np.load(dataset+'_inception_moments_train.npz')['sigma']

  data_mu_test = np.load(dataset+'_inception_moments_test.npz')['mu']
  data_sigma_test = np.load(dataset+'_inception_moments_test.npz')['sigma']
  # Load network
  ...
  def get_inception_metrics(sample, num_inception_images, num_splits=10, 
    ...
    if onlyZ:
        pool, logits = accumulate_inception_activations_onlyZ(sample, net, num_inception_images)
    else:
    ...
    if no_fid:
      ...
      if use_torch:
        print('... calculate FID using torch...')
        FID_train = torch_calculate_frechet_distance(mu, sigma, torch.tensor(data_mu_train).float().cuda(), torch.tensor(data_sigma_train).float().cuda())
        FID_train = float(FID_train.cpu().numpy())
        FID_test = torch_calculate_frechet_distance(mu, sigma, torch.tensor(data_mu_test).float().cuda(), torch.tensor(data_sigma_test).float().cuda())
        FID_test = float(FID_test.cpu().numpy())
      else:
        print('... calculate FID using numpy...')
        FID_train = numpy_calculate_frechet_distance(mu, sigma, data_mu_train, data_sigma_train)
        FID_test = numpy_calculate_frechet_distance(mu, sigma, data_mu_test, data_sigma_test)
    # Delete mu, sigma, pool, logits, and labels, just in case
    del mu, sigma, pool, logits
    return IS_mean, IS_std, FID_train, FID_test
