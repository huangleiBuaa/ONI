...
def name_from_config(config):
  ...
  'lr%2.1e' % config.lr,
  'nDis%d' % config.disc_iters,
  'bm%1.3f' % config.beta1,
  'bv%1.3f' % config.beta2,
  'seed%d' % config.seed,
  'T%d' % config.T if config.model.find('ONI') != -1 else None,
  'NS%2.2f' % config.NScale if config.model.find('ONI') != -1 else None,
  ]
  if item is not None])
  # dogball
  return name
...
# Convenience function to prepare a z vector: 4-dimesnon-batchsizexnzx1x1
def prepare_z(G_batch_size, dim_z, device='cuda',
                z_var=1.0):
  z_ = Distribution(torch.randn(G_batch_size, dim_z, 1, 1, requires_grad=False))
  z_.init_distribution('normal', mean=0, var=z_var)
  z_ = z_.to(device, torch.float32)
  return z_


# Convenience function to prepare a z vector: 2-dimesnon-batchsizexnz
def prepare_z_dense(G_batch_size, dim_z, device='cuda',
              z_var=1.0):
  z_ = Distribution(torch.randn(G_batch_size, dim_z, requires_grad=False))
  z_.init_distribution('normal', mean=0, var=z_var)
  z_ = z_.to(device, torch.float32)
